...
/**
 * Connects to the lockdown service on the device.
 * @param device The idevice_t instance.
 * @param client Receives the lockdown client.
 * @param label A label for the client, usually your app name.
 * @return LOCKDOWN_E_SUCCESS on success.
 */
lockdownd_error_t lockdownd_client_new(idevice_t device, lockdownd_client_t* client, const char* label);

/**
 * Free the lockdown client.
 */
void lockdownd_client_free3(lockdownd_client_t client);

/**
 * Starts a lockdown service by name.
 * @param client Lockdown client.
 * @param service_name Service identifier (e.g. "com.apple.afc").
 * @param service Receives the service descriptor.
 * @return LOCKDOWN_E_SUCCESS on success.
 */
lockdownd_error_t lockdownd_start_service(lockdownd_client_t client, const char* service_name, lockdownd_service_descriptor_t* service);

/**
 * Free the service descriptor.
 */
void lockdownd_service_descriptor_free3(lockdownd_service_descriptor_t service);

/**
 * Pair the device. Allocates a pair record on success.
 * @param client Lockdown client.
 * @param record Receives the pair record pointer.
 * @return LOCKDOWN_E_SUCCESS on success.
 */
lockdownd_error_t lockdownd_pair2(lockdownd_client_t client, lockdownd_pair_record_t* record);

/**
 * Validate the pairing with the device.
 * @param client Lockdown client.
 * @param record The pair record.
 * @return LOCKDOWN_E_SUCCESS on success.
 */
lockdownd_error_t lockdownd_validate_pair(lockdownd_client_t client, lockdownd_pair_record_t record);

/**
 * Free the pair record allocated by lockdownd_pair.
 */
void lockdownd_pair_record_free(lockdownd_pair_record_t record);

/**
 * Reads a saved pair record from disk.
 * @param udid Device UDID string.
 * @param record Receives the pair record pointer.
 * @return LOCKDOWN_E_SUCCESS on success.
 */
lockdownd_error_t lockdownd_read_pair_record(const char* udid, lockdownd_pair_record_t* record);

/**
 * Writes a pair record to disk.
 * @param udid Device UDID string.
 * @param record The pair record to save.
 * @return LOCKDOWN_E_SUCCESS on success.
 */
lockdownd_error_t lockdownd_save_pair_record(const char* udid, lockdownd_pair_record_t record);
